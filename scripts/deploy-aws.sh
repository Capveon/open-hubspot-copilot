#!/usr/bin/env bash
# Build linux/arm64, push to Capveon ECR, roll the copilot ECS service.
# Run from the open-hubspot-copilot repo root. AWS_PROFILE=capveon.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

AWS_PROFILE="${AWS_PROFILE:-capveon}"
AWS_REGION="${AWS_REGION:-us-east-1}"
CLUSTER="${CLUSTER:-capveon-prod}"
SERVICE="${SERVICE:-copilot}"
TFVARS="${TFVARS:-$ROOT/../monorepo/infra/terraform/terraform.tfvars}"

aws_cli() {
  docker run --rm \
    -e "AWS_PROFILE=${AWS_PROFILE}" \
    -e "AWS_REGION=${AWS_REGION}" \
    -e "AWS_DEFAULT_REGION=${AWS_REGION}" \
    -v "${HOME}/.aws:/root/.aws:ro" \
    -v /tmp:/tmp \
    amazon/aws-cli "$@"
}

CLERK_PUBLISHABLE_KEY="${NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY:-}"
if [ -z "$CLERK_PUBLISHABLE_KEY" ] && [ -f /tmp/osp-clerk-prod.env ]; then
  set -a
  # shellcheck disable=SC1091
  source /tmp/osp-clerk-prod.env
  set +a
  CLERK_PUBLISHABLE_KEY="${CLERK_PUBLISHABLE_KEY:-}"
fi
if [ -z "$CLERK_PUBLISHABLE_KEY" ] && [ -f "$TFVARS" ]; then
  CLERK_PUBLISHABLE_KEY="$(python3 - "$TFVARS" <<'PY'
import re, sys
text = open(sys.argv[1]).read()
m = re.search(r'practice_clerk_publishable_key\s*=\s*"([^"]+)"', text)
print(m.group(1) if m else "")
PY
)"
fi
if [ -z "$CLERK_PUBLISHABLE_KEY" ]; then
  echo "Need NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY, /tmp/osp-clerk-prod.env, or practice_clerk_publishable_key in terraform.tfvars" >&2
  exit 1
fi

if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  TAG="$(git rev-parse --short HEAD)"
else
  TAG="$(date +%Y%m%d%H%M%S)"
fi

REGISTRY="$(aws_cli ecr describe-repositories --repository-names "capveon-prod-${SERVICE}" \
  --query 'repositories[0].repositoryUri' --output text)"
IMAGE="${REGISTRY}:${TAG}"

echo "Building ${IMAGE}"
docker build --platform linux/arm64 --provenance=false --sbom=false \
  --build-arg "NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=${CLERK_PUBLISHABLE_KEY}" \
  -t "$IMAGE" \
  -t "${REGISTRY}:latest" \
  .

echo "Pushing"
aws_cli ecr get-login-password --region "$AWS_REGION" \
  | docker login --username AWS --password-stdin "${REGISTRY%%/*}"
docker push "$IMAGE"
docker push "${REGISTRY}:latest"

echo "Rolling ECS"
TASK_ARN="$(aws_cli ecs describe-services --cluster "$CLUSTER" --services "capveon-prod-${SERVICE}" \
  --query 'services[0].taskDefinition' --output text)"
aws_cli ecs describe-task-definition --task-definition "$TASK_ARN" \
  --query 'taskDefinition' > /tmp/ohc-td.json
python3 - "$IMAGE" "$SERVICE" <<'PY'
import json, sys
image, service = sys.argv[1], sys.argv[2]
td = json.load(open("/tmp/ohc-td.json"))
for key in (
    "taskDefinitionArn", "revision", "status", "requiresAttributes",
    "compatibilities", "registeredAt", "registeredBy",
):
    td.pop(key, None)
for container in td.get("containerDefinitions", []):
    if container.get("name") == service:
        container["image"] = image
json.dump(td, open("/tmp/ohc-td-new.json", "w"))
PY
NEW_ARN="$(aws_cli ecs register-task-definition --cli-input-json file:///tmp/ohc-td-new.json \
  --query 'taskDefinition.taskDefinitionArn' --output text)"
aws_cli ecs update-service --cluster "$CLUSTER" --service "capveon-prod-${SERVICE}" \
  --task-definition "$NEW_ARN" >/dev/null
aws_cli ecs wait services-stable --cluster "$CLUSTER" --services "capveon-prod-${SERVICE}"
echo "Deployed ${IMAGE}"
